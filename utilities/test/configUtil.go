...
func GetCmdWithConfig() *cobra.Command {
	...
	cmd.Flags().Int(constants.FlagCASPWaitTime, int(constants.DefaultCASPWaitTime.Seconds()), "casp signature wait time")
	...
	cmd.Flags().Int(constants.FlagCASPMaxAttempts, int(constants.DefaultCASPMaxAttempts), "max attempts for getting signature")
