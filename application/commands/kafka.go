...
func KafkaRootCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:                        "kafka",
		Short:                      "kafka root command",
		DisableFlagParsing:         true,
		SuggestionsMinimumDistance: 1,
		RunE:                       func(cmd *cobra.Command, args []string) error { return errors.New("expect a subcommand") },
	}
	cmd.AddCommand(InitializeKafkaCommand())
	cmd.PersistentFlags().String(utils.FlagKafkaHome, utils.DefaultKafkaHome, "The kafka configuration file directory")
	return cmd
}

func InitializeKafkaCommand() *cobra.Command {
	initCmd := &cobra.Command{
		Use:   "init",
		Short: "Init kafka configuration file",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Println("generated configuration file at ", filepath.Join(homeDir, "kafkaConfig.toml"))
