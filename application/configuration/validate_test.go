...
func TestValidateEthereum(t *testing.T) {
	...
	ethConfig.LiquidStakingAddress = common.HexToAddress("0x0000000000000000000000000000000000000001")
	err = ethConfig.validate()
	require.Equal(t, fmt.Errorf("empty token wrapper contract address"), err)

	ethConfig.TokenWrapperAddress = common.HexToAddress("0x0000000000000000000000000000000000000001")
	err = ethConfig.validate()
...
func TestValidateKafka(t *testing.T) {
	...
	testKafkaConfig.TopicDetails.ReplicationFactor = 1
	err = testKafkaConfig.validate()
	require.Equal(t, "num participants has to be atleast 1", err.Error())

	testKafkaConfig.TopicDetails.NumPartitions = 1
	err = testKafkaConfig.validate()
...
func TestConfig(t *testing.T) {
	...
	c.Ethereum.LiquidStakingAddress = common.HexToAddress("0x0000000000000000000000000000000000000001")
	c.Ethereum.TokenWrapperAddress = common.HexToAddress("0x0000000000000000000000000000000000000001")
	...
	c.Kafka.TopicDetails.ReplicationFactor = 1
	c.Kafka.TopicDetails.NumPartitions = 1
