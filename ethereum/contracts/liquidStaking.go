...
func onStake(kafkaState utils.KafkaState, protoCodec *codec.ProtoCodec, arguments []interface{}) error {
	...
	stakeMsg := stakingTypes.NewMsgDelegate(configuration.GetAppConfiguration().Tendermint.PStakeAddress, constants2.Validator1, sdkTypes.NewCoin(configuration.GetAppConfiguration().PStakeDenom, sdkTypes.NewInt(amount.Int64())))
...
func onUnStake(kafkaState utils.KafkaState, protoCodec *codec.ProtoCodec, arguments []interface{}) error {
	amount := arguments[1].(*big.Int)
	unStakeMsg := stakingTypes.NewMsgUndelegate(configuration.GetAppConfiguration().Tendermint.PStakeAddress, constants2.Validator1, sdkTypes.NewCoin(configuration.GetAppConfiguration().PStakeDenom, sdkTypes.NewInt(amount.Int64())))
